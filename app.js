...
bot.start((ctx) => {
  try {
    ctx
      .reply(
        "Welcome! this is WEB to pdf  bot. Send any webpage url to save as pdf. Github : https://github.com/khushhnood/Webpdf-bot.git"
      )
      .catch((err) => {
        if (err) {
          console.log("err");
...
bot.on("text", async (ctx) => {
  ...
  try {
    if (validate(url)) {
      ...
      ctx.reply(
        "This does not seems to be a valid link! Please provide a valid URL."
      );
    }
  } catch (error) {
